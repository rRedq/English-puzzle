...
import { type StorageAccess, type WordPosition, DataJson } from './types/interfaces';
...
export default class App extends CreateElement {
  ...
  public startGame(obj?: WordPosition, isModal?: boolean): void {
    if (this.game) {
      this.game.clearGame();
    }
    this.game = new Game(this, obj);
    if (isModal && obj) {
      this.appendChildren([this.game, new Modal(obj)]);
    } else {
